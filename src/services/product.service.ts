...
function formatProductResponse(product: Product): FormattedProduct {
  return {
    id: product.id,
...
export class ProductService {
  ...
  async lookupOpenFoodFacts(barcode: string) {
    ...
    try {
      const response = await axios.get<OpenFoodFactsApiResponse>(
   ...
   */
  async getCritical(user: AuthUser, explicitTenantId?: string) {
   ...
   */
  async transferStock(id: string, quantity: number, user: AuthUser) {
    if (quantity <= 0) {
    ...
    if (product.depotQty < quantity) {
      throw new AppError(
        `Estoque no depósito insuficiente para transferência. Disponível no depósito: ${product.depotQty}, Solicitado: ${quantity}`,
        400
      );
    }
   ...
   */
  async processSale(data: PosSaleInput, user: AuthUser) {
    const tenantId = this.resolveTenantId(user, data.tenantId);
    ...
    return prisma.$transaction(async (tx) => {
      ...
      for (const item of data.items) {
        ...
        if (!product || product.tenantId !== tenantId) {
          throw new AppError(
            `Produto ${item.productId || item.barcode} não encontrado no catálogo desta empresa`,
            404
          );
        }
        ...
        if (product.shelfQty < qty) {
          throw new AppError(
            `Estoque na gôndola insuficiente para "${product.name}". Disponível na gôndola: ${product.shelfQty}, Solicitado: ${qty}`,
            400
          );
        }
