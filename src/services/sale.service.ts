import { Prisma, Product, PaymentMethod, TenantPlan } from '@prisma/client';
...
export class SaleService {
   ...
   */
  async processSale(data: CreateSaleInput, user: AuthUser) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const updatedProductsList: Array<{
        id: string;
        name: string;
        barcode: string;
        soldQty: number;
        remainingShelfQty: number;
        previousShelfQty: number;
        depotQty: number;
      }> = [];
   ...
   */
  async getAll(user: AuthUser, filters: ListSalesFilter = {}) {
