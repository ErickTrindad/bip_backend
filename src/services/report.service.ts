import { Product, TenantPlan } from '@prisma/client';
...
interface AbcFilterParams extends ReportFilterParams {
  sortBy?: 'revenue' | 'margin' | 'turnover' | 'salesVolume';
  limit?: number;
}

interface MatrixFilterParams extends ReportFilterParams {
  classification?: 'ESTRELA' | 'ALTO_GIRO' | 'GERADOR_MARGEM' | 'LENTO_ABAIXO_MARGEM';
}

interface ReplenishmentFilterParams extends ReportFilterParams {
  leadTimeDays?: number;
  safetyStockDays?: number;
  status?: 'CRITICO_RUPTURA' | 'COMPRA_URGENTE' | 'ATENCAO' | 'ESTAVEL' | 'EXCESSO';
}

interface SpaceOptimizationFilterParams extends ReportFilterParams {
...
export class ReportService {
   ...
   */
  private async getTenantWithPlan(tenantId: string) {
   ...
   */
  private async getActiveProducts(tenantId: string, category?: string): Promise<Product[]> {
   ...
   */
  private async aggregateProductSales(
    tenantId: string,
    startDate: Date,
    endDate: Date
  ): Promise<Map<string, AggregatedProductSales>> {
    ...
    const salesMap = new Map<string, AggregatedProductSales>();
   ...
   */
  async getAbcReport(user: AuthUser, params: AbcFilterParams) {
    ...
    const { startDate, endDate, maxDaysAllowed } = normalizeDateRangeForPlan(
      plan,
      params.startDate,
      params.endDate
    );
    ...
    const enrichedProducts = products.map((product) => {
      ...
      let marginClass: 'ALTA' | 'MEDIA' | 'BAIXA' = 'MEDIA';
      if (marginPercentage >= 40) marginClass = 'ALTA';
      else if (marginPercentage <= 25) marginClass = 'BAIXA';

      // Classificação de Giro Real
      let turnoverClass: 'ALTO' | 'MEDIO' | 'BAIXO' | 'SEM_GIRO' = 'MEDIO';
      if (totalUnitsSold === 0) turnoverClass = 'SEM_GIRO';
      else if (turnoverRatio >= 1.5) turnoverClass = 'ALTO';
      else if (turnoverRatio <= 0.5) turnoverClass = 'BAIXO';
   ...
   */
  async getMatrixReport(user: AuthUser, params: MatrixFilterParams) {
    const tenantId = this.resolveTenantId(user, params.tenantId);
    const tenant = await this.getTenantWithPlan(tenantId);
    const plan: TenantPlan = tenant.plan || 'FREE';
    ...
    const items = products.map((product) => {
      ...
      let quadrant: 'ESTRELA' | 'ALTO_GIRO' | 'GERADOR_MARGEM' | 'LENTO_ABAIXO_MARGEM';
   ...
   */
  async getReplenishmentReport(user: AuthUser, params: ReplenishmentFilterParams) {
    const tenantId = this.resolveTenantId(user, params.tenantId);
    const tenant = await this.getTenantWithPlan(tenantId);
    const plan: TenantPlan = tenant.plan || 'FREE';
    ...
    const items = products.map((product) => {
      ...
      let status: 'CRITICO_RUPTURA' | 'COMPRA_URGENTE' | 'ATENCAO' | 'ESTAVEL' | 'EXCESSO';
      let statusLabel: string;
      let urgencyLevel: 'ALTA' | 'MEDIA' | 'BAIXA' | 'NENHUMA';
   ...
   */
  async getSpaceOptimizationReport(user: AuthUser, params: SpaceOptimizationFilterParams) {
    const tenantId = this.resolveTenantId(user, params.tenantId);
    const tenant = await this.getTenantWithPlan(tenantId);
    const plan: TenantPlan = tenant.plan || 'FREE';
    ...
    const items = enriched.map(({ product, totalRevenue, turnoverRatio, marginPercentage, allocatedShelf, shelfSharePercentage }) => {
      ...
      let recommendedAction: 'EXPANDIR_GONDOLA' | 'MANTER' | 'REDUZIR_GONDOLA' | 'REAVALIAR_MIX';
   ...
   */
  async getExecutiveOverview(user: AuthUser, explicitTenantId?: string) {
