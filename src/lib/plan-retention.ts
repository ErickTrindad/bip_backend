...
export const RETENTION_DAYS_BY_PLAN: Record<TenantPlan, number> = {
  FREE: 30,
  PRO: 90,
  PREMIUM: 365,
};
 ...
 */
export function getPlanRetentionDays(plan: TenantPlan = 'FREE'): number {
 ...
 */
export function calculatePlanCutoffDate(plan: TenantPlan = 'FREE', referenceDate: Date = new Date()): Date {
 ...
 */
export function normalizeDateRangeForPlan(
  plan: TenantPlan = 'FREE',
  requestedStartDate?: Date,
  requestedEndDate?: Date,
  referenceDate: Date = new Date()
): { startDate: Date; endDate: Date; planCutoffDate: Date; maxDaysAllowed: number } {
