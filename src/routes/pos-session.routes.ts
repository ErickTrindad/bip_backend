...
import {
  ...
  createPosSessionResponseSchema,
  validatePosSessionParamsSchema,
  validatePosSessionQuerySchema,
  validatePosSessionResponseSchema,
  closePosSessionResponseSchema,
} from '../schemas/pos-session.schema.js';

export const posSessionRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  ...
  typedApp.post(
    '/pos/sessions/pair',
    {
      ...
      schema: {
        ...
        description:
          'Gera um token efêmero de 30 minutos e canal Realtime único para que a câmera do celular se conecte como leitor de código de barras.',
        security: [{ bearerAuth: [] }],
  ...
  typedApp.get(
    '/pos/sessions/:sessionId/validate',
    {
      schema: {
        tags: ['PDV / Scanner Remoto'],
        summary: 'Validar sessão de pareamento no mobile',
        description:
          'Usado pelo celular ao abrir o link do QR Code para checar se a sessão está ativa e obter os metadados do canal de broadcast antes de ativar a câmera.',
