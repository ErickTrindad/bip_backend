...
export class ProductController {
  ...
  async getCritical(request: FastifyRequest, reply: FastifyReply) {
    const user = request.user!;
  ...
  async lookupOpenFoodFacts(request: FastifyRequest, reply: FastifyReply) {
  ...
  async getByBarcode(request: FastifyRequest, reply: FastifyReply) {
    const user = request.user!;
    const parseParams = productBarcodeParamsSchema.safeParse(request.params);
    const query = request.query as { tenantId?: string } | undefined;
  ...
  async processSale(request: FastifyRequest, reply: FastifyReply) {
    const user = request.user!;
