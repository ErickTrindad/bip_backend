...
import { createPosSessionBodySchema, validatePosSessionParamsSchema, validatePosSessionQuerySchema, } from '../schemas/pos-session.schema.js';
export class PosSessionController {
    ...
    async validate(request, reply) {
        ...
        try {
            const result = await posSessionService.validatePairingSession(parseParams.data.sessionId, parseQuery.data.token);
