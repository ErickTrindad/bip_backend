...
Você DEVE responder ESTRITAMENTE em formato JSON:
{
  ...
  "explanation": "Explicação em português simples e claro do que foi compreendido"
        ...
        const isScanCommand = normalizedTranscription.includes('varredura') ||
        ...
        const batchConfig = parsed.batchConfig || {
            operation: 'TRANSFER',
            scope: isScanCommand ? 'CRITICAL_ONLY' : 'ALL',
            from: 'depot',
            to: 'shelf',
            quantityRule: isScanCommand ? 'DEFICIT' : 'ALL',
        };
        const actionList = [];
                ...
                if (item.action === 'STOCK_ENTRY' && item.quantity) {
                    ...
                    if (isToDepot && (item.from === 'shelf' || item.to === 'depot')) {
                        if (targetProduct.shelfQty < qty) {
                            throw new AppError(`Estoque insuficiente na gôndola para transferir ao depósito. Gôndola de "${targetProduct.name}" possui ${targetProduct.shelfQty} un.`, 400);
                        }
