...
Você DEVE responder ESTRITAMENTE em formato JSON:
{
  ...
  "explanation": "Resumo amigável em português explicando todas as ações que serão realizadas para cada produto"
            ...
            ? rawActions.map((act) => ({
                ...
                price: act.price !== undefined && act.price !== null
                    ...
                    : act.action === 'UPDATE_PRODUCT'
                        ? resolvedPrice
                        : undefined,
                depotQty: act.depotQty !== undefined && act.depotQty !== null
                    ? Number(act.depotQty)
                    : undefined,
                shelfQty: act.shelfQty !== undefined && act.shelfQty !== null
                    ? Number(act.shelfQty)
                    : undefined,
            }))
        ...
        if (intent === 'REPLENISH_ALL_CRITICAL' && user.tenantId) {
            ...
            const criticalProducts = activeProductsInDepot.filter((prod) => prod.shelfQty <= prod.shelfMinQty);
            ...
            if (criticalProducts.length === 0) {
                ...
                for (const prod of criticalProducts) {
                    const transferQty = Math.min(Math.max(1, prod.shelfMinQty - prod.shelfQty), prod.depotQty);
                    const summary = {
                        id: prod.id,
                        name: prod.name,
